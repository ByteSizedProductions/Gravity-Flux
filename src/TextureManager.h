...
public:
	...
	void drawFromSheet(const std::string& id, int srcX, int srcY, int srcW, int srcH, int oldSize, int newSize, int x, int y, double angle = 0, int alpha = 255, bool centered = false, SDL_RendererFlip flip = SDL_FLIP_NONE);
	...
	void playAnimationOnce(const std::string& sprite_sheet_name, Animation& animation, int x, int y, float speed_factor, double angle, int alpha, bool centered = false, SDL_RendererFlip flip = SDL_FLIP_NONE);
	bool checkAnimationDone(Animation& animation);
	int checkAnimationFrame(Animation& animation);
